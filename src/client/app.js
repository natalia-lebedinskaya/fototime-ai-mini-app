...
/* CHATGPT_FORCE_HISTORY_SYNC_20260611_START */
(() => {
  if (window.__FT_FORCE_HISTORY_SYNC_20260611__) return;
  window.__FT_FORCE_HISTORY_SYNC_20260611__ = true;

  const STORAGE_KEY = 'ft_saved_generations_v6';
  ...
  function writeSavedGenerations(items) {
  ...
  function saveGeneration(item) {
    ...
    if (String(item.resultUrl).includes('/assets/mock-result.svg')) return;

    const list = readSavedGenerations();
    const merged = [
      item,
      ...list.filter((x) => String(x.id) !== String(item.id) && String(x.resultUrl) !== String(item.resultUrl))
    ].sort((a, b) => String(b.createdAt || '').localeCompare(String(a.createdAt || '')));

    writeSavedGenerations(merged);
  }

  function mergeGenerations(serverGenerations) {
    const local = readSavedGenerations();
    const map = new Map();

    [...local, ...(Array.isArray(serverGenerations) ? serverGenerations : [])]
      .filter((g) => g && g.resultUrl && !String(g.resultUrl).includes('/assets/mock-result.svg'))
      .forEach((g) => {
        const key = String(g.id || g.resultUrl);
  ...
  async function fetchLatestGenerationFromState(ctx) {
    try {
      ...
      const res = await window.__FT_ORIGINAL_FETCH__(`/api/fototime/state?${params.toString()}`);
      const json = await res.json().catch(() => null);
      if (!json || !Array.isArray(json.generations) || !json.generations.length) return null;

      const matched = [...json.generations]
        .sort((a, b) => String(b.createdAt || '').localeCompare(String(a.createdAt || '')))
        .find((g) => {
          if (!g || !g.resultUrl) return false;
          if (ctx.styleId && String(g.styleId || '') !== String(ctx.styleId)) return false;
          return true;
        }) || json.generations[0];

      if (!matched || !matched.resultUrl) return null;

      return {
        id: matched.id || `state_${Date.now()}`,
        styleId: matched.styleId || ctx.styleId || '',
        styleTitle: matched.styleTitle || ctx.styleTitle || '',
        provider: matched.provider || ctx.provider || 'AI',
        participant: matched.participant || ctx.participant || '',
        resultUrl: matched.resultUrl,
        sourceUrl: matched.sourceUrl || '',
        cost: Number(matched.cost || 40),
  ...
  function buildGenerationFromResponse(json, ctx) {
    ...
    if (!resultUrl || String(resultUrl).includes('/assets/mock-result.svg')) return null;

    return {
      id: generation.id || json?.generationId || `local_${Date.now()}`,
      styleId: generation.styleId || ctx.styleId || '',
      styleTitle: generation.styleTitle || ctx.styleTitle || '',
      provider: generation.provider || ctx.provider || 'AI',
      participant: generation.participant || ctx.participant || '',
      resultUrl,
      sourceUrl: generation.sourceUrl || '',
      cost: Number(generation.cost || json?.cost || json?.costCredits || 40),
      createdAt: generation.createdAt || json?.createdAt || new Date().toISOString(),
      status: 'success'
    };
  }

  if (!window.__FT_ORIGINAL_FETCH__) {
    window.__FT_ORIGINAL_FETCH__ = window.fetch.bind(window);
  }
  ...
  window.fetch = async (input, init = {}) => {
    ...
    if (url.includes('/api/generate') && method === 'POST' && init.body instanceof FormData) {
      const fd = init.body;
      const ctx = {
        styleId: String(fd.get('styleId') || fd.get('style') || ''),
        styleTitle: String(fd.get('styleTitle') || fd.get('title') || ''),
        provider: String(fd.get('provider') || fd.get('styleProvider') || 'AI'),
        participant: String(fd.get('participant') || fd.get('participantId') || ''),
      };

      const res = await window.__FT_ORIGINAL_FETCH__(input, init);
      ...
      try {
        ...
        if (res.ok && json && json.ok !== false) {
          let generation = buildGenerationFromResponse(json, ctx);

          if (!generation) {
            generation = await fetchLatestGenerationFromState(ctx);
          }
    ...
    if (url.includes('/api/fototime/state')) {
      try {
        ...
        json.generations = mergeGenerations(json.generations);
...
/* CHATGPT_FORCE_HISTORY_SYNC_20260611_END */



/* CHATGPT_RUNTIME_FIX_20260611_START */
(() => {
  if (window.__FT_RUNTIME_FIX_20260611__) return;
  window.__FT_RUNTIME_FIX_20260611__ = true;

  const STORAGE_KEY = 'ft_saved_generations_v5';
  const PUBLIC_STYLES_URL = 'https://api.cyberphotobooth.ru/api/public/styles';

  const LOCAL_STYLE_META = {
    '1001': { title: 'Атлантида', provider: 'Nano Banana', aliases: ['Atlantida', 'Atlantis', 'Атлантида Платона'] },
    ...
    '2002': { title: 'Кибернетика', provider: 'FLUX.2', aliases: ['Cybernetics', 'WhiteCybernetics', 'Кибернетика'] },
  ...
  async function getPublicStyles() {
    if (publicStylesCache) return publicStylesCache;
    try {
      const res = await fetch(PUBLIC_STYLES_URL, { headers: { Accept: 'application/json' } });
      const data = await res.json().catch(() => []);
      publicStylesCache = Array.isArray(data)
        ? data
  ...
  function findPublicStyle(localStyle, publicStyles) {
    const meta = LOCAL_STYLE_META[String(localStyle?.id || '').trim()] || null;
    const candidates = [];
    if (localStyle?.title) candidates.push(localStyle.title);
    if (meta?.title) candidates.push(meta.title);
    if (Array.isArray(meta?.aliases)) candidates.push(...meta.aliases);

    const needles = candidates.map(norm).filter(Boolean);

    for (const needle of needles) {
      const found =
        publicStyles.find((s) => norm(s?.display_name_ru) === needle) ||
        publicStyles.find((s) => norm(s?.display_name_en) === needle) ||
        publicStyles.find((s) => norm(s?.name) === needle) ||
        publicStyles.find((s) => {
          const vals = [s?.display_name_ru, s?.display_name_en, s?.name].map(norm).filter(Boolean);
          return vals.some((v) => v.includes(needle) || needle.includes(v));
        });
  ...
  function enrichStyle(style, publicStyles) {
    const meta = LOCAL_STYLE_META[String(style?.id || '').trim()] || null;
    const found = findPublicStyle(style, publicStyles);

    const preview =
      found?.preview_url_female ||
      found?.preview_url_male ||
      found?.preview_url ||
      style.preview ||
      '';
    ...
    return {
      ...style,
      preview,
      provider: meta?.provider || style.provider || ''
    };
  }
  ...
  function buildGenerationFromResponse(json, ctx) {
    ...
    const meta = LOCAL_STYLE_META[String(ctx.styleId || generation.styleId || '').trim()] || null;

    return {
      id: generation.id || json?.generationId || `local_${Date.now()}`,
      styleId: generation.styleId || ctx.styleId || '',
      styleTitle: generation.styleTitle || ctx.styleTitle || meta?.title || '',
      provider: meta?.provider || generation.provider || ctx.provider || 'AI',
      participant: generation.participant || ctx.participant || '',
      resultUrl,
      sourceUrl: generation.sourceUrl || '',
      cost: Number(generation.cost || json?.cost || json?.costCredits || 40),
  ...
  window.fetch = async (input, init = {}) => {
    ...
    if (url.includes('/api/generate') && method === 'POST' && init.body instanceof FormData) {
      ...
      const ctx = {
        styleId: String(fd.get('styleId') || fd.get('style') || ''),
        styleTitle: String(fd.get('styleTitle') || fd.get('title') || ''),
        provider: String(fd.get('provider') || fd.get('styleProvider') || ''),
        participant: String(fd.get('participant') || fd.get('participantId') || ''),
      };
      ...
      try {
        ...
        const json = await clone.json().catch(() => null);
        if (res.ok && json && json.ok !== false) {
          const generation = buildGenerationFromResponse(json, ctx);
          if (generation) saveGeneration(generation);
        }
      } catch (_) {}
    ...
    if (url.includes('/api/fototime/state')) {
      try {
        ...
        if (Array.isArray(json.styles)) {
          json.styles = json.styles.map((style) => enrichStyle(style, publicStyles));
        }
...
/* CHATGPT_RUNTIME_FIX_20260611_END */



/* CHATGPT_PREVIEW_ENRICH_20260611_START */
(() => {
  if (window.__FT_PREVIEW_ENRICH_INSTALLED__) return;
  window.__FT_PREVIEW_ENRICH_INSTALLED__ = true;

  const FT_LOCAL_STYLE_PREVIEW_MAP = {
    '1001': { title: 'Атлантида', aliases: ['Atlantida', 'Atlantis'] },
    '1002': { title: 'Барби', aliases: ['Barbie'] },
    '1003': { title: 'Баблгам', aliases: ['Bubblegum', 'Bablgam'] },
    '1004': { title: 'Бизнес', aliases: ['Business'] },
    '1005': { title: 'Рождество', aliases: ['Christmas', 'Рождество'] },
    '1006': { title: 'Комикс', aliases: ['Comics', 'Comic'] },
    '1007': { title: 'Киберпанк', aliases: ['Cyberpunk'] },
    '1008': { title: 'Дубай', aliases: ['Dubai'] },
    '1009': { title: 'Лесная сказка', aliases: ['Forest Tale', 'Fairy Forest'] },
    '2001': { title: 'Поле боя', aliases: ['23FevrBattlefield', 'Battlefield', 'Поле боя'] },
    '2002': { title: 'Кибернетика', aliases: ['Cybernetics', 'Кибернетика'] },
    '2003': { title: 'Алхимик', aliases: ['Alchemist', 'Алхимик'] },
    '2004': { title: 'Пираты', aliases: ['Pirates', 'Пираты'] },
    '2005': { title: 'Кабаре', aliases: ['Cabaret', 'Кабаре'] },
    '3001': { title: 'White rabbits', aliases: ['White rabbits'] },
    '3002': { title: 'Diamonds', aliases: ['Diamonds'] },
    '3003': { title: 'Luxor', aliases: ['Luxor'] },
    '3004': { title: 'Biker NB', aliases: ['Biker NB', 'Biker'] },
    '3005': { title: 'Business 001', aliases: ['Business 001', 'Business'] },
    '3006': { title: 'Пустыня', aliases: ['desert', 'Desert', 'Пустыня'] }
  };

  let __ftPublicStylesCache = null;
  ...
  async function getPublicStyles() {
    if (__ftPublicStylesCache) return __ftPublicStylesCache;
    try {
      const res = await fetch('https://api.cyberphotobooth.ru/api/public/styles', {
        headers: { Accept: 'application/json' }
      });
      const data = await res.json().catch(() => []);
      __ftPublicStylesCache = Array.isArray(data)
        ? data
        : Array.isArray(data?.styles)
          ? data.styles
          : Array.isArray(data?.items)
            ? data.items
            : [];
      return __ftPublicStylesCache;
    } catch (_) {
      __ftPublicStylesCache = [];
      return __ftPublicStylesCache;
    }
  }

  function findPreview(style, publicStyles) {
    const local = FT_LOCAL_STYLE_PREVIEW_MAP[String(style?.id || '').trim()] || null;
    const candidates = [];
    if (style?.title) candidates.push(style.title);
    if (local?.title) candidates.push(local.title);
    if (Array.isArray(local?.aliases)) candidates.push(...local.aliases);

    const needles = candidates.map(ftNorm).filter(Boolean);

    for (const needle of needles) {
      const found =
        publicStyles.find((s) => ftNorm(s?.display_name_ru) === needle) ||
        publicStyles.find((s) => ftNorm(s?.display_name_en) === needle) ||
        publicStyles.find((s) => ftNorm(s?.name) === needle) ||
        publicStyles.find((s) => {
          const vals = [s?.display_name_ru, s?.display_name_en, s?.name].map(ftNorm).filter(Boolean);
          return vals.some((v) => v.includes(needle) || needle.includes(v));
        });
  ...
  window.fetch = async (...args) => {
    ...
    try {
      ...
      const isState = url.includes('/api/fototime/state');

      if (!isState) return res;

      const cloned = res.clone();
      const json = await cloned.json().catch(() => null);
      if (!json || !Array.isArray(json.styles)) return res;

      const publicStyles = await getPublicStyles();

      json.styles = json.styles.map((style) => ({
        ...style,
        preview: findPreview(style, publicStyles)
      }));
...
/* CHATGPT_PREVIEW_ENRICH_20260611_END */
