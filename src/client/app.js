...
window.FT_DEPLOY_VERSION = "fototime-prod-20260610-141454";
...
(() => {
  ...
  async function loadState() {
    try {
      ...
      if (!state.selectedStyle && state.styles.length) {
      ...
      if (!state.selectedStyle) state.selectedStyle = state.styles[0];
    }
  }
  ...
  async function generate() {
    ...
    try {
      ...
      if (json.generation) {
        state.generations = [
          json.generation,
          ...state.generations.filter((g) => g.id !== json.generation.id),
        ];
      }
...
/* FT_V16_POLISH_START */
(() => {
  const FT_VERSION = "fototime-prod-20260610-173500-v16-polish";
  ...
  function runPolish() {
    ...
    fixGenerationEnoughText();
    addVersionCard();
    enhanceSupportLinks();
    enhanceGeneratedCards();
    addNotificationHelper();
    enhanceAdminUsers();
    retryBrokenFototimeImages();
    markUnreadCards();
  }
  ...
  document.addEventListener("click", (event) => {
    ...
    if (target.textContent?.toLowerCase().includes("авторизация и бонусы")) {
      setTimeout(() => {
        const modal = qa("section,article,div").find((el) => textIncludes(el, "авторизация") && textIncludes(el, "бонус"));
        if (modal) modal.classList.add("ft-readable-modal");
      }, 50);
    }

    if (target.textContent?.toLowerCase().includes("как авторизоваться")) {
      setTimeout(() => {
        const modal = qa("section,article,div").find((el) => textIncludes(el, "telegram") && textIncludes(el, "авторизац"));
        if (modal) modal.classList.add("ft-readable-modal");
      }, 50);
    }
  });
  ...
  const observer = new MutationObserver(() => {
    clearTimeout(window.__ftPolishTimer);
    window.__ftPolishTimer = setTimeout(runPolish, 80);
  });
